...
const corsOptions = {
  origin: ["https://fb-v2-eta.vercel.app"], // exact Vercel frontend URL
...
app.post('/api/preview', async (req, res) => {
  try {
    ...
    const contentType = parser.detectContentType(url);
    ...
    const preview = {
      ...
      preview: {
        contentType: contentType,
        previewImage: `https://graph.facebook.com/${extractFacebookId(url)}/picture?type=large`,
...
// Start server
app.listen(PORT, () => {
  console.log(`Facebook Downloader Backend running on port ${PORT}`);
